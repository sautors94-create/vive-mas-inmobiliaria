...
// Muchas más colonias para variar los textos
const colonias = [
  'Centro', 'Jardines del Valle', 'Residencial San Ángel', 'Villa Florida', 'Las Américas',
  'Del Valle', 'Polanco', 'Condesa', 'Roma Norte', 'Lomas de Chapultepec',
  'Satélite', 'Cuajimalpa', 'Del Carmen', 'Independencia', 'Reforma',
  'Progreso', 'Bugambilias', 'Los Pinos', 'Alamos', 'San Miguel'
];

const adjetivos = ['Excelente', 'Hermosa', 'Amplia', 'Moderna', 'Lujosa', 'Cómoda', 'Elegante', 'Funcional', 'Preciosa', 'Inigualable'];
...
async function seedDatabase() {
  try {
    ...
    // 600 propiedades / 32 estados = ~19 por estado
    const propsPorEstado = 19;

    estadosMexico.forEach((estado) => {
      for (let i = 0; i < propsPorEstado; i++) {
        ...
        // Variación de precio mucho más amplia (desde muy barato hasta ultra lujoso)
        const variacion = 0.5 + (Math.random() * 1.5); // Entre 0.5x y 2.0x
        ...
        propiedadesAGuardar.push({
          titulo: `${adjetivo} ${tipo} en ${operacion} en ${colonia}`,
          descripcion: `${adjetivo.toLowerCase()} ${tipo} ubicado en la colonia ${colonia} de ${estado.ciudad}, ${estado.nombre}. Cuenta con excelentes acabados, ubicación privilegiada y amplios espacios. Ideal para vivir o invertir.`,
          precio: precio,
          operacion: operacion,
          tipo: tipo,
          ubicacion: {
            ...
            colonia: colonia,
            direccion: `Calle ${adjetivo} #${Math.floor(Math.random() * 900) + 100}`,
            lat: estado.lat + (Math.random() * 0.04 - 0.02), // Radio un poco más grande
            lng: estado.lng + (Math.random() * 0.04 - 0.02)
          },
          caracteristicas: {
            recamaras: (tipo === 'terreno' || tipo === 'local') ? 0 : Math.floor(Math.random() * 4) + 1,
            banos: (tipo === 'terreno') ? 0 : Math.floor(Math.random() * 3) + 1,
            mediosBanos: (tipo === 'casa' || tipo === 'departamento') ? Math.floor(Math.random() * 2) : 0,
            estacionamientos: (tipo === 'terreno') ? 0 : Math.floor(Math.random() * 3) + 1,
            m2: tipo === 'terreno' ? Math.floor(Math.random() * 800) + 100 : Math.floor(Math.random() * 200) + 40
          },
          propietario: new mongoose.Types.ObjectId(USUARIO_ID),
          fotos: fotosPorTipo[tipo],
          status: 'aprobada', 
          planPeso: 0,
          destacada: Math.random() > 0.9, // 10% de probabilidad de ser destacada
          createdAt: new Date(),
          updatedAt: new Date()
        });
      }
    ...
    console.log(`⏳ Insertando ${propiedadesAGuardar.length} propiedades (por lotes de 100)...`);
    for (let i = 0; i < propiedadesAGuardar.length; i += 100) {
      const lote = propiedadesAGuardar.slice(i, i + 100);
      await collection.insertMany(lote);
      console.log(`✅ Lote insertado: ${i + lote.length} de ${propiedadesAGuardar.length}`);
    }

    console.log(`🎉 ¡Éxito total! Se agregaron ${propiedadesAGuardar.length} propiedades nuevas.`);
    process.exit(0);
