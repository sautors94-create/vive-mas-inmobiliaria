...
// Ruta dinámica: /renta/departamentos/cdmx/coyoacan
exports.showDynamicSEOPage = async (req, res) => {
  try {
    const operacion = req.params.operacion.toLowerCase(); // renta o venta
    const tipo = req.params.tipo.toLowerCase(); // departamentos, casas, terrenos, locales
    const estado = req.params.estado ? req.params.estado.toLowerCase() : null;
    const ciudad = req.params.ciudad ? req.params.ciudad.toLowerCase() : null;

    // Mapear plural URL -> valor real del enum en el schema (singular)
    const tipoMap = { departamentos: 'departamento', casas: 'casa', terrenos: 'terreno', locales: 'local' };
    const tipoBD = tipoMap[tipo] || tipo;

    // Construir filtro con los campos REALES del schema de Property
    let filter = {
      operacion,
      tipo: tipoBD,
      status: 'aprobada',
    ...
    const tituloCapitalizado = (str) => (str ? str.charAt(0).toUpperCase() + str.slice(1) : '');
    ...
    <body>
        <div class="container">
            ...
            <div class="grid">
                ...
                ${propiedades.map(p => `
                    <div class="card">
                        <img src="${(p.fotos && p.fotos[0]) || 'https://via.placeholder.com/400x200?text=Sin+Imagen'}" alt="${p.titulo}">
                        <div class="card-body">
                            <div class="price">$${Number(p.precio).toLocaleString('es-MX')} MXN</div>
                            <div class="loc">📍 ${[p.ubicacion?.colonia, p.ubicacion?.ciudad].filter(Boolean).join(', ')}</div>
                            <a href="${p.slug ? `/p/${p.slug}` : `/pages/propiedad.html?id=${p._id}`}" class="btn">Ver Detalles</a>
                        </div>
                    </div>
                `).join('')}
            </div>
    ...
    console.error('Error en showDynamicSEOPage:', error);
